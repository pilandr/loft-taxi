...
const InputCard = ({ input, meta, label, valueInput, onChangeInput, id, classNameInput }) => {
  return (
    <>
      <label className="form-profile__label-text" htmlFor={input.name}>{label}</label>
      <input {...input} className={classNameInput} value={valueInput} onChange={onChangeInput} id={id} />
      {meta.error && meta.visited && !meta.active && (
        <pre className="input__error--name">{meta.error}</pre>
      )}
    </>
  );
};
                ...
                <form className="form-profile" onSubmit={handleSubmit}>
                  ...
                  <div className="form-profile__body">
                    <div className="form-profile__left">
                      <Field classNameInput="form-profile__input input" id="name" type="text" name="name" size="28" valueInput={this.state.card.cardName} onChangeInput={this.handleNameChange} label="Имя владельца" component={InputCard} />
                      <Field classNameInput="form-profile__input input" id="number" type="text" name="number" size="28" valueInput={this.state.card.cardNumber} onChangeInput={this.handleNumberChange} label="Номер карты" component={InputCard} />
                      <div className="form-profile__row">
                        <div className="form-profile__column">
                          <Field classNameInput="form-profile__input form-profile__input--row input" id="date" type="text" name="date" size="28" valueInput={this.state.card.expiryDate} onChangeInput={this.handleDateChange} label="MM/YY" component={InputCard} />
                        </div>
                        <div className="form-profile__column">
                          <Field classNameInput="form-profile__input form-profile__input--row input" id="cvc" type="text" name="cvc" size="28" valueInput={this.state.card.cvc} onChangeInput={this.handleCVCChange} label="CVC" component={InputCard} />
                        </div>
                      </div>
                    </div>
